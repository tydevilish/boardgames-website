...
<head>
    ...
    <title>จัดการบอร์ดเกม - Board & Card Game</title>
...
<body class="bg-gray-50 font-sans">
    ...
    <div class="ml-64 p-8">
        ...
        <div class="flex justify-between items-center mb-8">
            ...
            <button onclick="showModal('boardgameModal')"
                ...
                <span style="font-family: 'Kanit', sans-serif;">เพิ่มบอร์ดเกมใหม่</span>
            </button>
        </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-600"
                            style="font-family: 'Kanit', sans-serif;">ชื่อบอร์ดเกม</th>
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-600"
                            style="font-family: 'Kanit', sans-serif;">จำนวนผู้เล่น</th>
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-600"
                            style="font-family: 'Kanit', sans-serif;">ความยาก</th>
                        <th class="px-6 py-3 text-left text-sm font-semibold text-gray-600"
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    $stmt = $conn->query("SELECT * FROM boardgames ORDER BY created_at DESC");
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-gray-600">
                                <span class="px-3 py-1 text-xs font-medium rounded-full 
                                    <?php echo $difficulty_colors[$boardgame['difficulty']] ?? 'bg-gray-100 text-gray-800'; ?>">
                                    <?php echo $difficulty_text[$boardgame['difficulty']] ?? 'ไม่ระบุ'; ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 text-gray-600">
                                <?php echo htmlspecialchars($boardgame['play_time']); ?>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="px-3 py-1 text-xs font-medium rounded-full <?php echo $boardgame['status'] === 'published' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'; ?>">
                                    <?php echo $boardgame['status'] === 'published' ? 'เผยแพร่' : 'ฉบับร่าง'; ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right space-x-2">
                                <button onclick="showModal('boardgameModal', <?php echo $boardgame['id']; ?>)"
                                    class="text-blue-600 hover:text-blue-800">
                                    <i class="ri-edit-line text-lg"></i>
                                </button>
                                <button onclick="confirmDelete(<?php echo $boardgame['id']; ?>)"
    ...
    <!-- Add/Edit Boardgame Modal -->
    <div id="boardgameModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
            ...
            <div class="inline-block w-full max-w-4xl text-left align-middle transition-all transform bg-white rounded-xl shadow-2xl relative">
                <div class="p-8">
                    <h3 class="text-2xl font-bold text-gray-800 mb-6" style="font-family: 'Kanit', sans-serif;">
                        <span id="modalTitle">เพิ่มบอร์ดเกมใหม่</span>
                    </h3>
                    <form id="boardgameForm" action="./actions/save-boardgame.php" method="POST"
                        enctype="multipart/form-data">
                        <input type="hidden" name="boardgame_id" id="boardgameId">
                        <div class="space-y-6">
                            <!-- รูปภาพ -->
                            <div>
                            ...
                            <div>
                                ...
                                <input type="text" name="player_count" id="playerCountInput" required placeholder="เช่น 2-4"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="difficulty" id="difficultyInput"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <div>
                                ...
                                <input type="text" name="play_time" id="playTimeInput" required placeholder="เช่น 30-60 "
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            </div>
                        </div>

                        <div class="mt-8 flex justify-end space-x-4">
                            <button type="button" onclick="closeModal('boardgameModal')"
    ...
    <script>
        ...
        // Show modal function
        function showModal(modalId, boardgameId = null) {
            ...
            if (boardgameId) {
                ...
                fetch(`actions/get-boardgame.php?id=${boardgameId}`)
                    .then(response => response.json())
                    .then(data => {
                        document.getElementById('boardgameId').value = data.id;
                        ...
                        document.getElementById('modalTitle').textContent = 'แก้ไขบอร์ดเกม';
                    });
            } else {
                document.getElementById('modalTitle').textContent = 'เพิ่มบอร์ดเกมใหม่';
            }
        }
        ...
        // Delete confirmation
        function confirmDelete(boardgameId) {
            if (confirm('คุณแน่ใจหรือไม่ที่จะลบบอร์ดเกมนี้?')) {
                window.location.href = `actions/delete-boardgame.php?id=${boardgameId}`;
            }
        }
    </script>
</body>
