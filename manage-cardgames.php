...
<head>
    ...
    <title>จัดการการ์ดเกม - Board & Card Game</title>
...
<body class="bg-gray-50 font-sans ">
    ...
    <div class="ml-64 p-8">
        ...
        <div class="flex justify-between items-center mb-8">
            ...
            <button onclick="showModal('cardgameModal')"
                ...
                <span style="font-family: 'Kanit', sans-serif;">เพิ่มการ์ดเกมใหม่</span>
            </button>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    $stmt = $conn->query("SELECT * FROM cardgames ORDER BY created_at DESC");
                    ...
                    ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <img src="<?php echo htmlspecialchars($cardgame['image_url']); ?>" alt="Cardgame thumbnail"
                                    class="h-16 w-24 object-cover rounded">
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-800" style="font-family: 'Kanit', sans-serif;">
                                    <?php echo htmlspecialchars($cardgame['title']); ?>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-gray-600">
                                <?php echo htmlspecialchars($cardgame['player_count']); ?> คน
                            </td>
                            <td class="px-6 py-4 text-gray-600">
                                <span
                                    class="px-3 py-1 text-xs font-medium rounded-full <?php echo $difficulty_colors[$cardgame['difficulty']] ?? 'bg-gray-100 text-gray-800'; ?>">
                                    <?php echo $difficulty_text[$cardgame['difficulty']] ?? 'ไม่ระบุ'; ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 text-gray-600">
            ...
            <div class="inline-block w-full max-w-4xl text-left align-middle transition-all transform bg-white rounded-xl shadow-2xl relative">
                <div class="p-8">
                    <h3 class="text-2xl font-bold text-gray-800 mb-6" style="font-family: 'Kanit', sans-serif;">
                        <span id="modalTitle">เพิ่มการ์ดเกมใหม่</span>
                    </h3>
                    <form id="cardgameForm" action="./actions/save-cardgame.php" method="POST" enctype="multipart/form-data">
                        <input type="hidden" name="cardgame_id" id="cardgameId">
                        <div class="space-y-6">
                            <!-- รูปภาพ -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2" style="font-family: 'Kanit', sans-serif;">
                                    รูปภาพ
                                </label>
                                <div class="flex items-center space-x-4">
                                    <img id="imagePreview" src="#" alt="Preview" class="h-32 w-48 object-cover rounded hidden">
    ...
    <script>
        ...
        function showModal(modalId, cardgameId = null) {
            ...
            if (cardgameId) {
                // Load cardgame data for editing
                fetch(`actions/get-cardgame.php?id=${cardgameId}`)
                    ...
                    .then(data => {
                        ...
                        document.getElementById('modalTitle').textContent = 'แก้ไขการ์ดเกม';
                    });
            } else {
                document.getElementById('modalTitle').textContent = 'เพิ่มการ์ดเกมใหม่';
            }
        }
        ...
        function confirmDelete(cardgameId) {
            if (confirm('คุณแน่ใจหรือไม่ที่จะลบการ์ดเกมนี้?')) {
                window.location.href = `actions/delete-cardgame.php?id=${cardgameId}`;
            }
        }
    </script>
</body>
