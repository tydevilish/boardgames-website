...
if (isset($_GET['id'])) {
    try {
        ...
        $stmt = $conn->prepare("DELETE FROM how_to_play_boardgames WHERE id = :id");
        $stmt->execute([':id' => $_GET['id']]);

        header('Location: ../manage-how-to-play-boardgames.php?success=delete');
        exit();

    } catch(PDOException $e) {
        error_log("Error deleting boardgame: " . $e->getMessage());
        header('Location: ../manage-how-to-play-boardgames.php?error=delete');
        exit();
    }
}
?> 